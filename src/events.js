 ...
 */
import {
  FORMATS, STYLES_3D, EMOTIONS, HAND_STATES, POSITIONS, FIXED_SHAPE, FIXED_COLOR, PANEL_CAM_REF_DIST_3D,
...
import {
  clamp, wrapAngle, getBBox, tracéBBox, getElementDepth, repairElementBase3D, getFormat, getHandles,
...
import {
  setScene3DCallbacks, disposeAllRigs3D, panelAutoDepthPivot3D, panelCamBasis3D, panelDepthToDistance3D,
  ...
  useObjectFormat3D, worldFloorToScreen, worldPointToPageXY3D, worldToPageXY, panelSceneCache3D,
  slabMeshCache3D, tracéMeshCache3D, mergedBuildWallRigCache3D, getRoomScreenBBoxFrom2DProjections,
  getBuildingJunctionCorners,
} from './scene3d.js';
import {
  setIOCallbacks, hasElectronAPI, applyProjectData, startAutosave, openRenameEntityModal, confirmAction,
  alertAction, loadPoseLibrary, loadDismissedPoses, restoreBuiltinPoses, missingBuiltinPoseCount,
} from './io.js';
import {
  setDrawCallbacks, uniqueDefaultName, addRoomWallElement, stopBuildMode, buildToolCreateWallSegment,
  buildToolClose, getPanelPoints, bubbleTailVisible, getBubbleTailTip, getRoomBoundingBoxXZ,
  getBuildingBoundingBoxXZ, drawCurrentPage, renderAll, scheduleDrawCurrentPage, flushDrawCurrentPage,
...
import {
  ...
  updateWallFaceFieldForSelectedWall, openRoomModal, closeRoomModal, openBuildingModal,
  closeBuildingModal, openTracéModal, openTerrainModal, refreshRoomPreview, refreshBuildingPreview,
...
async function deleteVolume(ti){
...
function applyRoomScaleFixed(roomIds, page, panel, sx, sz, fixedWX, fixedWZ, origWalls, origSlabs) {
...
function moveJunctionToWorld(jx, jz, newJx, newJz, buildingRoomIds, page, panel, tol = 0.12) {
  const connected = [];
  page.objects.forEach(o => {
