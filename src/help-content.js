...
export const HELP_MANUAL_EN = [
  ...
  { title: 'Characters', paragraphs: [
    ...
    'The left menu also has a "Characters" section that opens the editor with no target: compose a pose and save it to your library. In that mode the "Apply" button is absent — there is no character to apply it to — and the title says so.',
    'In the editor the view ORBITS around the character: hold the RIGHT mouse button to turn around it, scroll wheel to zoom. The view cannot be moved sideways — a lone figure is already centred, and the framing starts afresh every time the editor opens.',
    ...
    'In the editor, click a joint point on the figure to select it: the matching slider group expands and is highlighted. Clicking empty space deselects it. Expanding a group works the other way round and selects its joint on the figure. Keep the left button held on a joint point and drag to adjust it directly. Only ONE field moves at a time: the one highlighted more strongly in the right-hand panel. The gesture FOLLOWS the orientation of the model — turn the figure and the drag direction adapts. When the rotation faces you head-on, no straight direction means anything any more, so you turn around the joint point instead, like a knob. When the joint has several (a head, a shoulder), the scroll wheel switches from one to the next. A selected joint therefore takes the wheel over from the zoom — click empty space to deselect it and get the zoom back.',
...
export const HELP_MANUAL_FR = [
  ...
  { title: 'Personnages', paragraphs: [
    ...
    "Le menu de gauche comporte aussi une section « Personnages » qui ouvre l'éditeur sans cible : composez une pose et enregistrez-la dans votre bibliothèque. Dans ce mode le bouton « Appliquer » est absent — il n'y a aucun Personnage à qui l'appliquer — et le titre l'indique.",
    "Dans l'éditeur, la vue ORBITE autour du Personnage : maintenez le clic DROIT pour en faire le tour, molette pour zoomer. La vue ne se déplace pas latéralement — une figure seule est déjà centrée, et le cadrage repart de zéro à chaque ouverture de l'éditeur.",
    ...
    "Dans l'éditeur, cliquez un point d'articulation sur la figure pour le sélectionner : le groupe de curseurs correspondant se déplie et se surligne. Cliquer dans le vide désélectionne. Déplier un groupe fait l'inverse et sélectionne son articulation sur la figure. Maintenez le clic gauche sur un point d'articulation et glissez pour la régler directement. Un SEUL champ bouge à la fois : celui que le panneau de droite surligne plus fortement. Le geste SUIT l'orientation du modèle : faites pivoter la figure, la direction de glisser s'adapte. Quand la rotation vous fait face, plus aucune direction droite n'a de sens : on tourne alors autour du point d'articulation, comme un bouton. Quand l'articulation en a plusieurs (une tête, une épaule), la molette passe de l'un à l'autre. Une articulation sélectionnée confisque donc la molette au zoom — cliquez dans le vide pour la désélectionner et retrouver le zoom.",
