...
const fichiers = readdirSync(DOCS).filter(f => f.endsWith('.md')).sort();
const anglais = fichiers.filter(f => !f.endsWith('.fr.md'));
const francais = fichiers.filter(f => f.endsWith('.fr.md'));
const contenu = (f) => readFileSync(join(DOCS, f), 'utf8');
const jumeau = (f) => f.endsWith('.fr.md') ? f.replace(/\.fr\.md$/, '.md') : f.replace(/\.md$/, '.fr.md');
  ...
  test('RÉGRESSION : aucun lien mort entre documents', () => {
    ...
    fichiers.forEach(f => {
      liensDe(contenu(f)).forEach(cible => {
        const chemin = join(DOCS, cible.split('#')[0]);
...
describe('docs/ : les deux versions ont la même charpente', () => {
  ...
  test('l\'index cite tous les documents, dans les deux langues', () => {
    ...
    [['README.md', anglais], ['README.fr.md', francais]].forEach(([index, liste]) => {
      const cites = new Set(liensDe(contenu(index)).map(c => c.split('#')[0]));
      liste.filter(f => f !== index).forEach(f => {
        assert.ok(cites.has(f), `« ${f} » n'est cité nulle part dans « ${index} »`);
      });
    });
  });
});
...
describe('docs/ : les renvois depuis le code visent une note existante', () => {
  test('RÉGRESSION : « cf. docs/xxx.md » dans le code pointe vers un fichier réel', () => {
    ...
    // Ce fichier-ci est exclu du balayage : il cite « docs/xxx.md » comme EXEMPLE dans son propre
    ...
    sources.forEach(rel => {
      ...
      [...texte.matchAll(/docs\/([A-Za-z0-9._-]+\.md)/g)].forEach(m => {
        renvois++;
        assert.ok(existsSync(join(DOCS, m[1])),
          `${rel} renvoie à « docs/${m[1]} », qui n'existe pas`);
      });
    });
