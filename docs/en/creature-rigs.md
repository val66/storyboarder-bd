...
Settled in conversation, and moved into its own note:
[archetype-roles.md](archetype-roles.md). In one paragraph: the eighteen slots become a special case
of the chains, each segment carries a role, and it is the ARCHETYPE that defines the list of possible
roles. Without roles an archetype pose has nothing to aim at, a chain's name being free text local to
one file.
