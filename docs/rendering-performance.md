...
reported medians alongside means — the first frame of a session builds every rig and is several
...
| Phase | Share | Notes |
|---|---|---|
| WebGL render of a Panel | **64 %** | ~0.69 renders per frame — the 3D cache absorbs the rest |
...
| "Coalescing draw requests will absorb a large fraction of the work" | 1018 requests produced 1018 frames — it absorbed nothing in this campaign |
...
frame to protect a cache that hits nine times out of ten — not the WebGL render, which is already
guarded.
...
- **off by default**, switched from the console — an always-on probe measures part of its own cost;
